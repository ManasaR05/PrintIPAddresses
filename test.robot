...
*** Test Cases ***
Valid Input1
    ${result} =    Run Process    python3    ${IP_PRINT}    input1.json  # valid input file having IP address
    Should Be Equal As Strings    ${result.stdout}    192.168.101.101\n192.168.101.70\n192.168.101.153      strip_spaces=True   collapse_spaces=True

Valid Input2
    ${result} =    Run Process    python3    ${IP_PRINT}    input2.json  # valid input file having IP address and networkIPs
    Should Be Equal    ${result.stdout}    192.168.102.33 10.0.0.87\n192.168.103.74 10.0.0.77\n192.168.102.155 10.0.0.99

Missing File
    ${result} =    Run Process    python3    ${IP_PRINT}    nonexistent.json  # given file is not existing
...
Invalid JSON2
    Create File    invalid.json    {"key": "value""}  # extra closing quote
    ${result} =    Run Process    python3    ${IP_PRINT}    invalid.json
    Should Contain    ${result.stderr}    Invalid JSON

Missing vm_private_ips Key
    Create File    missing_key.json    {"some_other_key": "value"}  # required key in the json file is missing
